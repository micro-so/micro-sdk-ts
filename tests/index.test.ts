...
import { APIPromise } from 'micro/core/api-promise';

import util from 'node:util';
import Micro from 'micro';
import { APIUserAbortError } from 'micro';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['MICRO_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Micro) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new Micro({ apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['MICRO_LOG'] = 'debug';
      const client = new Micro({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['MICRO_LOG'] = 'not a log level';
      const client = new Micro({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'MICRO_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Micro({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['MICRO_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Micro({ baseURL: 'https://example.com', apiKey: 'My API Key' });
    ...
    test('in request options overridden by client options', () => {
      const client = new Micro({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['MICRO_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Micro({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Micro({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Micro({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['MICRO_API_KEY'] = 'My API Key';
    const client = new Micro();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['MICRO_API_KEY'] = 'another My API Key';
    const client = new Micro({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Micro({ apiKey: 'My API Key', fetch: testFetch });
