...
describe('logging', () => {
  ...
  teamID: 'My Team ID',
    ...
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'MICRO_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  teamID: 'My Team ID',
    ...
    // default
    const client2 = new Micro({ apiKey: 'My API Key', teamID: 'My Team ID' });
  ...
  test('with environment variable arguments', () => {
    ...
    const client = new Micro({ teamID: 'My Team ID' });
    expect(client.apiKey).toBe('My API Key');
    expect(client.teamID).toBe('My Team ID');
  });
