...
export interface ImageUploadParams extends ImageScope {
  file: Blob;
}
...
export class RecordImages extends APIResource {
  constructor(
    client: Micro,
    private readonly objectType: 'identity' | 'organization',
  ) {
    super(client);
  }
  ...
  async upload(id: string, params: ImageUploadParams, options?: RequestOptions): Promise<RecordImage> {
    if (!params.file.size || params.file.size > 5 * 1024 * 1024)
      throw new Error('Images must be between 1 byte and 5 MB');
    if (!['image/jpeg', 'image/png', 'image/gif', 'image/webp'].includes(params.file.type))
    ...
    const upload = await this.requestUpload(
      id,
      { ...scope, mime_type: params.file.type as ImageMimeType },
      stepOptions(options, invocationID, 'request'),
    );
    ...
    return this.complete(
      id,
      { ...scope, upload_id: upload.upload_id },
      stepOptions(options, upload.upload_id, 'complete'),
    );
  }
}
...
function stepOptions(
  ...
  const key =
    sourceKey.length + suffix.length <= 255 ?
      `${sourceKey}${suffix}`
