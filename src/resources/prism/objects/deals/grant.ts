...
export class Grant extends APIResource {
   ...
   * @example
   * ```ts
   * const grant = await client.prism.objects.deals.grant.update(
  ...
  update(
    ...
    return this._client.put(path`/v2/prism/${teamId}/deal/${dealID}/grant`, {
      body,
      ...options,
   ...
   * @example
   * ```ts
   * const grant = await client.prism.objects.deals.grant.get(
   *   '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',
   * );
   * ```
   */
  get(
    ...
    return this._client.get(path`/v2/prism/${teamId}/deal/${dealID}/grant`, options);
  }
}
