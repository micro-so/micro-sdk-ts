...
export class Documents extends APIResource {
   ...
   * ```ts
   * const document =
   *   await client.prism.objects.documents.create({
   *     default: {
   ...
   * ```ts
   * const document =
   *   await client.prism.objects.documents.update(
   ...
   * ```ts
   * const documents =
   *   await client.prism.objects.documents.list();
   * ```
   */
  list(
   ...
   * @example
   * ```ts
   * await client.prism.objects.documents.delete(
   *   '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',
   * );
   * ```
   */
   ...
   * ```ts
   * const response =
   *   await client.prism.objects.documents.bulkCreate({
   *     objects: [{}],
   *   });
   * ```
   */
   ...
   * ```ts
   * const response =
   *   await client.prism.objects.documents.bulkDelete({
   ...
   * ```ts
   * const response =
   *   await client.prism.objects.documents.bulkUpdate({
   ...
   * ```ts
   * const response =
   *   await client.prism.objects.documents.count();
   * ```
   */
  count(
   ...
   * ```ts
   * const response =
   *   await client.prism.objects.documents.duplicate(
   *     '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',
   *   );
   * ```
   */
   ...
   * @example
   * ```ts
   * const response = await client.prism.objects.documents.find(
   *   'value',
   *   { slug: 'slug' },
   * );
   * ```
   ...
   * @example
   * ```ts
   * const document = await client.prism.objects.documents.get(
   *   '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',
   * );
   * ```
   */
  ...
  query(params: DocumentQueryParams, options?: RequestOptions): APIPromise<DocumentQueryResponse> {
    const { teamId = this._client.teamID, 'Idempotency-Key': idempotencyKey, ...body } = params;
    return this._client.post(path`/v2/prism/${teamId}/document/query`, {
      body,
      ...options,
      headers: buildHeaders([
        { ...(idempotencyKey != null ? { 'Idempotency-Key': idempotencyKey } : undefined) },
   ...
   * ```ts
   * const response =
   *   await client.prism.objects.documents.restore(
   *     '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',
   *   );
   * ```
   */
   ...
   * ```ts
   * const response =
   *   await client.prism.objects.documents.upsert('value', {
   *     slug: 'slug',
   *   });
   * ```
   */
