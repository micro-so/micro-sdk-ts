...
export class Identities extends APIResource {
  images = new RecordImages(this._client, 'identity');
   ...
   * ```ts
   * const identity =
   *   await client.prism.objects.identities.create({
   *     default: {
   ...
   * ```ts
   * const identity =
   *   await client.prism.objects.identities.update(
   ...
   * ```ts
   * const identities =
   *   await client.prism.objects.identities.list();
   * ```
   */
  list(
   ...
   * @example
   * ```ts
   * await client.prism.objects.identities.delete(
   *   '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',
   * );
   * ```
   */
   ...
   * ```ts
   * const response =
   *   await client.prism.objects.identities.bulkCreate({
   *     objects: [{}],
   *   });
   * ```
   */
   ...
   * ```ts
   * const response =
   *   await client.prism.objects.identities.bulkDelete({
   ...
   * ```ts
   * const response =
   *   await client.prism.objects.identities.bulkUpdate({
   ...
   * ```ts
   * const response =
   *   await client.prism.objects.identities.count();
   * ```
   */
  count(
   ...
   * ```ts
   * const response =
   *   await client.prism.objects.identities.duplicate(
   *     '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',
   *   );
   * ```
   */
   ...
   * @example
   * ```ts
   * const response = await client.prism.objects.identities.find(
   *   'value',
   *   { slug: 'slug' },
   * );
   * ```
   ...
   * ```ts
   * const response =
   *   await client.prism.objects.identities.findOrCreate({
   *     match: { email_address: 'email_address' },
   *   });
   * ```
   */
  findOrCreate(
    params: IdentityFindOrCreateParams,
    options?: RequestOptions,
  ): APIPromise<IdentityFindOrCreateResponse> {
    const { teamId = this._client.teamID, 'Idempotency-Key': idempotencyKey, ...body } = params;
    return this._client.post(path`/v2/prism/${teamId}/identity/find-or-create`, {
      body,
      ...options,
      headers: buildHeaders([
        { ...(idempotencyKey != null ? { 'Idempotency-Key': idempotencyKey } : undefined) },
   ...
   * @example
   * ```ts
   * const identity = await client.prism.objects.identities.get(
   *   '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',
   * );
   * ```
   */
   ...
   * ```ts
   * const response =
   *   await client.prism.objects.identities.query({
   *     query: {
   *       select: [
   *         'full_name',
   *         'email',
  ...
  query(params: IdentityQueryParams, options?: RequestOptions): APIPromise<IdentityQueryResponse> {
    const { teamId = this._client.teamID, 'Idempotency-Key': idempotencyKey, ...body } = params;
    return this._client.post(path`/v2/prism/${teamId}/identity/query`, {
      body,
      ...options,
      headers: buildHeaders([
        { ...(idempotencyKey != null ? { 'Idempotency-Key': idempotencyKey } : undefined) },
   ...
   * ```ts
   * const response =
   *   await client.prism.objects.identities.restore(
   *     '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',
   *   );
   * ```
   */
   ...
   * ```ts
   * const response =
   *   await client.prism.objects.identities.upsert('value', {
   *     slug: 'slug',
   *   });
   * ```
   */
...
export interface IdentityFindOrCreateResponse {
  created: boolean;

  record: IdentityFindOrCreateResponse.Record;
}

export namespace IdentityFindOrCreateResponse {
  export interface Record {
    id: string;
...
export interface IdentityFindOrCreateParams {
  /**
   * Path param
   */
  teamId?: string;
   ...
   * Body param
   */
  match: IdentityFindOrCreateParams.Match;
...
export namespace IdentityFindOrCreateParams {
...
export declare namespace Identities {
  export {
    ...
    type IdentityFindOrCreateResponse as IdentityFindOrCreateResponse,
    ...
    type IdentityFindOrCreateParams as IdentityFindOrCreateParams,
