...
export class Grant extends APIResource {
   ...
   * ```ts
   * const grant =
   *   await client.prism.objects.engagements.grant.update(
  ...
  update(
    ...
    return this._client.put(path`/v2/prism/${teamId}/engagement/${engagementID}/grant`, {
      body,
      ...options,
   ...
   * ```ts
   * const grant =
   *   await client.prism.objects.engagements.grant.get(
   *     '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',
   *   );
   * ```
   */
  get(
    ...
    return this._client.get(path`/v2/prism/${teamId}/engagement/${engagementID}/grant`, options);
  }
}
