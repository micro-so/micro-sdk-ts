...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    markdown:
      "## create\n\n`client.prism.objects.contacts.create(teamId: string, default?: object, list?: object, Idempotency-Key?: string): { id: string; default?: object; list?: object; }`\n\n**post** `/v2/prism/{teamId}/contact`\n\nCreates a record. For `document`, writing `content` (or HTML) stores the property and reads back, but the in-app editor is CRDT-backed and will render a blank page until that document has been opened and saved in the app. Treat API-created docs as data records, not as collaboratively edited pages, unless you only need the stored property values.\n\n### Parameters\n\n- `teamId: string`\n\n- `default?: object`\n  Properties keyed by property slug. Values can be strings, numbers, booleans, arrays, or null. For select/multiselect properties, values may be option slugs or option UUIDs on write; option slugs are returned on read. Identity email_addresses accepts contact UUIDs or email strings on create/update. Emails use Micro normalization and resolve to contact links within the write transaction; creating an identity does not merge other identities. Arrays replace links; {_op: 'append'|'remove', values: [...]} changes only the specified links. Removing an email never creates a contact. Identity companies contains organization UUIDs, whose read access is checked when adding links.\n\n- `list?: object`\n\n- `Idempotency-Key?: string`\n\n### Returns\n\n- `{ id: string; default?: object; list?: object; }`\n  Object returned by reads (get/create/patch/restore). id is always present.\n\n  - `id: string`\n  - `default?: object`\n  - `list?: object`\n\n### Example\n\n```typescript\nimport Micro from '@micro-so/sdk';\n\nconst client = new Micro();\n\nconst contact = await client.prism.objects.contacts.create();\n\nconsole.log(contact);\n```",
    perLanguage: {
      typescript: {
  ...
  {
    name: 'find_or_create',
    endpoint: '/v2/prism/{teamId}/organization/find-or-create',
    httpMethod: 'post',
    summary: 'Find one visible person or company, or create it without updating matches',
    description:
      'Matches identity by normalized email_address or organization by canonical lowercase primary_domain. defaults is a flat slug/value object used only on creation; do not include the matching field. Returns 409 for multiple visible matches. Same-key calls through this endpoint serialize; ordinary creates and updates do not participate. Matching is workspace- and caller-access-scoped, not a global uniqueness guarantee.',
    stainlessPath: '(resource) prism.objects.organizations > (method) find_or_create',
    qualified: 'client.prism.objects.organizations.findOrCreate',
    params: [
      'teamId: string;',
      'match: { primary_domain: string; };',
      'defaults?: object;',
      'Idempotency-Key?: string;',
    ],
    response: '{ created: boolean; record: { id: string; }; }',
    markdown:
      "## find_or_create\n\n`client.prism.objects.organizations.findOrCreate(teamId: string, match: { primary_domain: string; }, defaults?: object, Idempotency-Key?: string): { created: boolean; record: object; }`\n\n**post** `/v2/prism/{teamId}/organization/find-or-create`\n\nMatches identity by normalized email_address or organization by canonical lowercase primary_domain. defaults is a flat slug/value object used only on creation; do not include the matching field. Returns 409 for multiple visible matches. Same-key calls through this endpoint serialize; ordinary creates and updates do not participate. Matching is workspace- and caller-access-scoped, not a global uniqueness guarantee.\n\n### Parameters\n\n- `teamId: string`\n\n- `match: { primary_domain: string; }`\n  - `primary_domain: string`\n\n- `defaults?: object`\n\n- `Idempotency-Key?: string`\n\n### Returns\n\n- `{ created: boolean; record: { id: string; }; }`\n\n  - `created: boolean`\n  - `record: { id: string; }`\n\n### Example\n\n```typescript\nimport Micro from '@micro-so/sdk';\n\nconst client = new Micro();\n\nconst response = await client.prism.objects.organizations.findOrCreate({ match: { primary_domain: 'primary_domain' } });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.prism.objects.organizations.findOrCreate',
        example:
          "import Micro from '@micro-so/sdk';\n\nconst client = new Micro({\n  teamID: 'My Team ID',\n  apiKey: process.env['MICRO_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.prism.objects.organizations.findOrCreate({\n  match: { primary_domain: 'primary_domain' },\n});\n\nconsole.log(response.created);",
      },
      python: {
        method: 'prism.objects.organizations.find_or_create',
        example:
          'import os\nfrom micro_so import Micro\n\nclient = Micro(\n    api_key=os.environ.get("MICRO_API_KEY"),  # This is the default and can be omitted\n)\nresponse = client.prism.objects.organizations.find_or_create(\n    match={\n        "primary_domain": "primary_domain"\n    },\n)\nprint(response.created)',
      },
      go: {
        method: 'client.Prism.Objects.Organizations.FindOrNew',
        example:
          'package main\n\nimport (\n\t"context"\n\t"fmt"\n\n\t"github.com/micro-so/micro-sdk-go"\n\t"github.com/micro-so/micro-sdk-go/option"\n)\n\nfunc main() {\n\tclient := micro.NewClient(\n\t\toption.WithAPIKey("My API Key"),\n\t\toption.WithTeamID("My Team ID"),\n\t)\n\tresponse, err := client.Prism.Objects.Organizations.FindOrNew(context.TODO(), micro.PrismObjectOrganizationFindOrNewParams{\n\t\tMatch: micro.F(micro.PrismObjectOrganizationFindOrNewParamsMatch{\n\t\t\tPrimaryDomain: micro.F("primary_domain"),\n\t\t}),\n\t})\n\tif err != nil {\n\t\tpanic(err.Error())\n\t}\n\tfmt.Printf("%+v\\n", response.Created)\n}\n',
      },
      http: {
        example:
          'curl https://developers.micro.so/v2/prism/$TEAM_ID/organization/find-or-create \\\n    -H \'Content-Type: application/json\' \\\n    -H "x-api-key: $MICRO_API_KEY" \\\n    -d \'{\n          "match": {\n            "primary_domain": "primary_domain"\n          }\n        }\'',
      },
    },
  },
  {
  ...
  {
    name: 'find_or_create',
    endpoint: '/v2/prism/{teamId}/identity/find-or-create',
    httpMethod: 'post',
    summary: 'Find one visible person or company, or create it without updating matches',
    description:
      'Matches identity by normalized email_address or organization by canonical lowercase primary_domain. defaults is a flat slug/value object used only on creation; do not include the matching field. Returns 409 for multiple visible matches. Same-key calls through this endpoint serialize; ordinary creates and updates do not participate. Matching is workspace- and caller-access-scoped, not a global uniqueness guarantee.',
    stainlessPath: '(resource) prism.objects.identities > (method) find_or_create',
    qualified: 'client.prism.objects.identities.findOrCreate',
    params: [
      'teamId: string;',
      'match: { email_address: string; };',
      'defaults?: object;',
      'Idempotency-Key?: string;',
    ],
    response: '{ created: boolean; record: { id: string; }; }',
    markdown:
      "## find_or_create\n\n`client.prism.objects.identities.findOrCreate(teamId: string, match: { email_address: string; }, defaults?: object, Idempotency-Key?: string): { created: boolean; record: object; }`\n\n**post** `/v2/prism/{teamId}/identity/find-or-create`\n\nMatches identity by normalized email_address or organization by canonical lowercase primary_domain. defaults is a flat slug/value object used only on creation; do not include the matching field. Returns 409 for multiple visible matches. Same-key calls through this endpoint serialize; ordinary creates and updates do not participate. Matching is workspace- and caller-access-scoped, not a global uniqueness guarantee.\n\n### Parameters\n\n- `teamId: string`\n\n- `match: { email_address: string; }`\n  - `email_address: string`\n\n- `defaults?: object`\n\n- `Idempotency-Key?: string`\n\n### Returns\n\n- `{ created: boolean; record: { id: string; }; }`\n\n  - `created: boolean`\n  - `record: { id: string; }`\n\n### Example\n\n```typescript\nimport Micro from '@micro-so/sdk';\n\nconst client = new Micro();\n\nconst response = await client.prism.objects.identities.findOrCreate({ match: { email_address: 'email_address' } });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.prism.objects.identities.findOrCreate',
        example:
          "import Micro from '@micro-so/sdk';\n\nconst client = new Micro({\n  teamID: 'My Team ID',\n  apiKey: process.env['MICRO_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.prism.objects.identities.findOrCreate({\n  match: { email_address: 'email_address' },\n});\n\nconsole.log(response.created);",
      },
      python: {
        method: 'prism.objects.identities.find_or_create',
        example:
          'import os\nfrom micro_so import Micro\n\nclient = Micro(\n    api_key=os.environ.get("MICRO_API_KEY"),  # This is the default and can be omitted\n)\nresponse = client.prism.objects.identities.find_or_create(\n    match={\n        "email_address": "email_address"\n    },\n)\nprint(response.created)',
      },
      go: {
        method: 'client.Prism.Objects.Identities.FindOrNew',
        example:
          'package main\n\nimport (\n\t"context"\n\t"fmt"\n\n\t"github.com/micro-so/micro-sdk-go"\n\t"github.com/micro-so/micro-sdk-go/option"\n)\n\nfunc main() {\n\tclient := micro.NewClient(\n\t\toption.WithAPIKey("My API Key"),\n\t\toption.WithTeamID("My Team ID"),\n\t)\n\tresponse, err := client.Prism.Objects.Identities.FindOrNew(context.TODO(), micro.PrismObjectIdentityFindOrNewParams{\n\t\tMatch: micro.F(micro.PrismObjectIdentityFindOrNewParamsMatch{\n\t\t\tEmailAddress: micro.F("email_address"),\n\t\t}),\n\t})\n\tif err != nil {\n\t\tpanic(err.Error())\n\t}\n\tfmt.Printf("%+v\\n", response.Created)\n}\n',
      },
      http: {
        example:
          'curl https://developers.micro.so/v2/prism/$TEAM_ID/identity/find-or-create \\\n    -H \'Content-Type: application/json\' \\\n    -H "x-api-key: $MICRO_API_KEY" \\\n    -d \'{\n          "match": {\n            "email_address": "email_address"\n          }\n        }\'',
      },
    },
  },
  {
  ...
  },
  {
    ...
    response:
      '{ data: { id: string; is_user_object?: boolean; properties?: object; source?: string[]; }[]; has_more: boolean; next_cursor?: string; total?: number; }',
    markdown:
      "## query\n\n`client.prism.objects.documents.query(teamId: string, query: { select: string[]; combinator?: 'AND' | 'OR'; cursor?: string; filter?: object[]; limit?: number; list_id?: string; page?: number; sort?: object[]; }, id?: string | string[], boxes?: string[], cursor?: string, deleted?: boolean, include_total?: boolean, sources?: string[], Idempotency-Key?: string): { data: object[]; has_more: boolean; next_cursor?: string; total?: number; }`\n\n**post** `/v2/prism/{teamId}/document/query`\n\nQuery\n\n### Parameters\n\n- `teamId: string`\n\n- `query: { select: string[]; combinator?: 'AND' | 'OR'; cursor?: string; filter?: object[]; limit?: number; list_id?: string; page?: number; sort?: object[]; }`\n  - `select: string[]`\n    Property slugs to select. Use dot notation for relationships (e.g. attendee.contact.first_name). `id` is always returned at the top level of each row and does not need to be selected.\n  - `combinator?: 'AND' | 'OR'`\n    Logical operator for combining filters\n  - `cursor?: string`\n    Opaque cursor from a previous response's `next_cursor`. Pass it back unchanged to fetch the next page. When set, `page` and `limit` are derived from the cursor and any explicit values are ignored.\n  - `filter?: object[]`\n    Filters as [{ slug: { operator: value } }]. For select/multiselect properties, values may be option slugs or option UUIDs.\n  - `limit?: number`\n    Maximum number of rows to return. Capped server-side at 50; requests above the cap are rejected.\n  - `list_id?: string`\n  - `page?: number`\n    Page number (1-based). Prefer `cursor`. Page-number pagination drifts under concurrent writes; use it only for one-shot exports.\n  - `sort?: object[]`\n    Sort order as [{ slug: direction }]. Array order determines sort priority\n\n- `id?: string | string[]`\n\n- `boxes?: string[]`\n\n- `cursor?: string`\n  Alternative location for the opaque cursor (a sibling of `query`). Use whichever feels more natural; if both are present, `query.cursor` wins.\n\n- `deleted?: boolean`\n\n- `include_total?: boolean`\n  When true, the response includes a `total` field with the unpaginated row count. Costs an additional pass over the result set — for unfiltered totals prefer `GET /v2/prism/{teamId}/{objectType}/count` instead.\n\n- `sources?: string[]`\n\n- `Idempotency-Key?: string`\n\n### Returns\n\n- `{ data: { id: string; is_user_object?: boolean; properties?: object; source?: string[]; }[]; has_more: boolean; next_cursor?: string; total?: number; }`\n\n  - `data: { id: string; is_user_object?: boolean; properties?: object; source?: string[]; }[]`\n  - `has_more: boolean`\n  - `next_cursor?: string`\n  - `total?: number`\n\n### Example\n\n```typescript\nimport Micro from '@micro-so/sdk';\n\nconst client = new Micro();\n\nconst response = await client.prism.objects.documents.query({ query: { select: ['full_name', 'email', 'title', 'organization'] } });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
