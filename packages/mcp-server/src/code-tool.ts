...
import { readEnv, readEnvOrError, requireValue } from "./util";
...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { MICRO_API_KEY: requireValue(readEnv('MICRO_API_KEY') ?? client.apiKey, 'set MICRO_API_KEY environment variable or provide apiKey client option'), MICRO_BASE_URL: readEnv('MICRO_BASE_URL') ?? client.baseURL ?? undefined };
  ...
  try {
    const resp = await new Promise<Response>((resolve, reject) => {
      ...
      const opts = {
          ...
          ...(client.teamID != null ? { teamID: client.teamID } : undefined),
          defaultHeaders: {
